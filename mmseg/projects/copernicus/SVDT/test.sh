...
export CONDA_ENV_NAME='mmdofa2'                                         # miniconda 虚拟环境名称，确保此环境可以正常运行你的项目。
export CUDA_VISIBLE_DEVICES='5'                                         # 指定训练使用的 GPU 索引，格式：0, 1, 2..., N。
export CONFIG_NAME='SVDT_copernicus-fm-base_upernet_e50-frozen'              # 训练启动配置名称，权重文件要求相同命名。
...
WORK_DIR="/tmp/work_dirs/$DATASET/upernet"
mkdir -p "$WORK_DIR"
