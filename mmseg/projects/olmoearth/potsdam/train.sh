...
export CUDA_VISIBLE_DEVICES='4'                                         # 指定训练使用的 GPU 索引，格式：0, 1, 2..., N。
export CONFIG_NAME='olmoearth-base_upernet_4xb4-80k_potsdam-rgb-p4-512x512_offline-linear'              # 训练启动配置名称，权重文件要求相同命名。
...
$PYINTERPRETER "$PWD/tools/train.py" "$SRCDIR/configs/$CONFIG_NAME.py" --work-dir "$WORK_DIR" --resume
