...
def build_neck_list(ops: list[dict], channel_list: list[int]) -> tuple[list[Neck], list[int]]:
    ...
    for cur_op in ops:
        op: Neck = globals()[cur_op["name"]](cur_channel_list, **{k: v for k, v in cur_op.items() if k != "name"})
        cur_channel_list = op.process_channel_list(cur_channel_list)
        op.channel_list = cur_channel_list
        neck_list.append(op)
    return torch.nn.Sequential(*neck_list), cur_channel_list


@MODELS.register_module()
class Neck4DOFA2(BaseModule):
    def __init__(self, necks_config: list[dict], in_channels: list[int], init_cfg=None):
        super().__init__(init_cfg=init_cfg)
        self.neck, self.cur_channel_list = build_neck_list(ops=necks_config, channel_list=in_channels)
        self.out_channels = self.cur_channel_list[-1]
