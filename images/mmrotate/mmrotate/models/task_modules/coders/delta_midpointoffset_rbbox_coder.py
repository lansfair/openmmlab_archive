...
def bbox2delta(proposals: Tensor,
    ...
    dx = (gx - px) / pw.clamp(min=1e-6)
    dy = (gy - py) / ph.clamp(min=1e-6)
    dw = torch.log((gw / pw.clamp(min=1e-6)).clamp(min=1e-16))
    dh = torch.log((gh / ph.clamp(min=1e-6)).clamp(min=1e-16))
    da = (ga - gx) / gw.clamp(min=1e-6)
    db = (gb - gy) / gh.clamp(min=1e-6)
...
def delta2bbox(rois: Tensor,
    ...
    diag_scale_factor = max_diag_len / diag_len.clamp(min=1e-6)
